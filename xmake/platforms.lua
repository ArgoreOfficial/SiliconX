...
function init_platform()
    ...
    if is_mode( "Debug", "Debug-nomt" ) then
        ...
        add_defines( "SIX_DEBUG" )
    elseif is_mode( "Release" ) then 
        set_symbols "debug"
        set_optimize "fast"
        set_strip "debug"

        add_defines( "SIX_RELEASE" )
    elseif is_mode( "Package" ) then 
        set_symbols "none"
        set_optimize "fastest"
        set_strip "all"

        add_defines( "SIX_PACKAGE" )
    end
    ...
    after_load(function (target)
        ...
        -- target:add( "defines", "SIX_PLATFORM_" .. string.upper( config.plat() ) )
        target:add( "defines", "SIX_ARCH_"     .. string.upper( config.arch() ) )
